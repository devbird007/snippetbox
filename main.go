...
func home(w http.ResponseWriter, r *http.Request) {
	...
	// If it doesn't use the http.NotFound() function to send a
	// 404 response to the client. Importantly, we then return
	// from the handler. If we don't return, the handler would
...
func snippetView(w http.ResponseWriter, r *http.Request) {
	...
	// string and try to convert it to an integer using the
	// strconv.Atoi() function. If it can't be converted to
	// an integer, or the value is less than 1, we return a 404
	...
	// with our response and write it to the http.ResponseWriter.
	fmt.Fprintf(w, "Display the specific snippet with ID %d...\n", id)
}
...
func main() {
	...
	// and exit. Note that any error returned by http.ListenAndServe() is always non-nil.
